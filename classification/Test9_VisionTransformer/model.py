...
class Mlp(nn.Module):
    """
    MLP as useed in Vision Transformer, MLP-Mixer and related  networks
    """
...
class Block(nn.Module):
    def __init__(self,
        ...
        self.drop_path = DropPath(drop_path_patio) if drop_path_patio > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_layer = int(dim * mlp_ratio)
        self.mlp = Mlp(dim, hidden_features=mlp_hidden_layer, act_layer=act_layer, drop=drop_ratio)
...
class VisionTransformer(nn.Module):
    def __init__(self,
        ...
        # torch.linspace(st,en,step) 形成等差序列
        dpr = [x.item() for x in torch.linspace(0, drop_path_patio, depth)]
    ...
    def forward_features(self, x):
        ...
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)  # [B, 196, 168]
        else:
